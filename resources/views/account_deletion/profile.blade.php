...
@section('title')
    ...
    @else
        Edit
    @endif
    {{ isset($guard) ? ucfirst($guard) : "User" }} Details
@endsection
@section('page-css')
                        ...
                        <form id="main" method="post" action="{{route('post:account:deletion:delete-account:logout')}}" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                <div class="form-group col-sm-12">
                                    ...
                                    <div class="card">
                                        <div class="card-header mr-auto ml-auto" style="width: fit-content">
                                            <h5>{{isset($guard) ? ucfirst($guard) : "User"}} Details</h5>
                                        </div>
                                        <div class="card-block">
                                            ...
                                            <div class="row">
                                                <div class="form-group mr-auto ml-auto col-sm-6">
                                                    ...
                                                    <div class="form-group row">
                                                        ...
                                                        <div class="col-sm-12">
                                                            <input type="text" class="form-control" name="store_name" id="store_name" readonly placeholder="Name"
                                                                   value="{{ (isset($user_details)) ? $user_details->first_name : old('store_name') }}">
                                                    ...
                                                    <div class="form-group row">
                                                        <label class="col-sm-12 col-form-label">Contact No.:<sup class="error">*</sup></label>
                                                        <div class="col-sm-12">
                                                    ...
                                                    <div class="form-group row">
                                                        <label class="col-sm-12 col-form-label">Gender:<sup
                                ...
                                <div class="form-group col-sm-12">
                                    <div class="row">
                                        <div class="col-sm-12">
                                            <center><button type="submit" id="delete-confirm" class="btn btn-primary m-b-0">Delete Account
                                                </button></center>
                                        </div>
                                    </div>
                                </div>
    ...
    <script>
        $(document).ready(function () {
            $('#delete-confirm').click(function (e) {
                ...
                var id = '{{ \Illuminate\Support\Facades\Auth::guard($guard)->id() }}';
                var guard = '{{ $guard }}';

                var form = $(this).closest('#main');
                let title = "Delete User Account";
                let text = "If you press yes, then it will be Deleted.";
                ...
                swal({
                    ...
                    if (isConfirm) {
                        $.ajax({
                            type: "post",
                            url: '{{ route('post:account:deletion:delete-account:logout') }}',
                            data: {
                                _token: "{{ csrf_token() }}",
                                id: id,
                                guard: guard
                            ...
                            success: function (result) {
                                if (result.success == true) {
                                    swal({
                                        ...
                                        // window.location = '/account-deletion/login';
                                        window.location.href='{{ route('get:account:deletion:login') }}';
                                    });
                        ...
                        return swal("Cancelled", "Your Account not Deleted", "error");
                    }
                });
            });
        });
