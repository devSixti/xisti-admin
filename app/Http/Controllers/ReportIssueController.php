...
//    }
    ...
    public function showReportIssues($type_of_list, Request $request)
    {
        ...
        if ($type_of_list == "customer") {
            ...
            $provider_type = 1;
            $user_list = User::query();
        } elseif ($type_of_list == "driver") {
            // For providers: provider_type is 3, use Provider model
            $provider_type = 2;
            $user_list = User::query()->where('is_driver_type',1);
        } else {
...
//            $user_list = $user_list->where('provider_type', $provider_type);
            ...
            if (request()->get("admin_role") == 4) {
                $user_list->where('users.area_id', request()->get("admin_city_id"));
            }
//        }
    ...
    public function updateReportIssuesStatus(Request $request)
    {
        ...
        $report_issue = ReportIssues::query()
            ...
            ->join('users', 'report_issues.provider_id', '=', 'users.id')
...
info($report_issue);
        ...
        if ($report_issue->status == 1) {
            ...
            $language = $report_issue->language != null ? $report_issue->language : 'es';
            $this->notificationClass->sendReportIssueResolvedNotification(
                (int) $report_issue->id,
                (string) $report_issue->device_token,
                (string) $report_issue->reference_no,
                $language
            );
            ...
            if ($general_settings != null && $general_settings->send_mail == 1) {
                $issue_category = $report_issue->ride_id != 0 ? $report_issue->order_no : "General Issue";
                try {
                ...
                $query->where(function ($q) use ($searchValue) {
                    if ($searchValue) {
                        $q->where('report_issues.description', 'like', "%$searchValue%")
                            ...
                            ->orWhere('user_ride_booking.ride_no', 'like', "%$searchValue%");
//                            ->orWhereRaw("(CASE
...
//                        END) LIKE ?", "%$searchValue%");
            ...
            if ($admin_role == 4) {
                ...
                $query->where(function ($q) use ($admin_city_id) {
                    $q->where('users.area_id', $admin_city_id)
                        ->orWhere('users.area_id', $admin_city_id);
                });
            }
            ...
            if ($request->created_by != null) {
                $query->where(function ($q) use ($request) {
                    $q->where('users.id', '=', $request->created_by);
                });
            }
            ...
            $records = $query->select('report_issues.*','users.first_name as created_by','user_ride_booking.ride_no')
//                ->selectRaw("
//            report_issues.*,
//            CASE
//                WHEN report_issues.provider_type = 0 THEN users.first_name
...
//        ")
            ...
            $data_arr = $records->map(function ($record, $index) use ($start) {
                ...
                return [
                    ...
                    "order_no" => $record->ride_no ? '#' . $record->ride_no : 'N/A',
    ...
    public function showReportDetails($id, $provider_id)
    {
        ...
        $report_issue = ReportIssues::query()
            ->select(
                'report_issues.*','users.first_name as created_by','user_ride_booking.ride_no'
            )
            ->leftJoin('user_ride_booking', 'user_ride_booking.id', '=', 'report_issues.ride_id')
            ->join('users', 'users.id', '=', 'report_issues.provider_id')
...
//        \Log::info($id);
        ...
        if (isset($report_issue->provider_type)) {
        ...
        if ($report_issue != null) {
            if (isset($report_issue->provider_type) && $report_issue->provider_type == 1) {
                $user_id = "u_" . $report_issue->provider_id;
            } else {
                $user_id = "u_" . $report_issue->provider_id;
            }
            $user_image = url('/assets/images/profile-images/customer/' . $user_details->avatar);
            $report_name = $report_issue->title;
        }

        if ($user_details->avatar == null) {
            $user_image = url('/assets/front/img/clients/default.png');
        }
...
//        \Log::info($chat_replace_domain);
    ...
    public function sendMessageNotification(Request $request)
    {
        ...
        $deviceToken = $devaicToken != null ? (string) $devaicToken->device_token : '';
        $isImage = (int) $request->get('is_image') !== 0;

        $this->notificationClass->sendReportIssueChatNotification(
            $deviceToken,
            (string) $request->get('report_id'),
            (string) $request->get('order_chat_number'),
            'Admin',
            (string) $request->get('user_message'),
            $isImage
        );
        return response()->json([
            "success" => true,
