...
class ReportIssueController extends Controller
{
    ...
    private function resolveProviderCheck(Request $request)
    {
        $providerType = (int) $request->get('provider_type', 1);
        if ($providerType === 1) {
            return $this->userClassapi->checkUserAllow($request->get('provider_id'), $request->get('access_token'));
        }

        return $this->userClassapi->checkDriverRegisterAllow($request->get('provider_id'));
    }

    private function failIfAuthError(mixed $check): ?\Illuminate\Http\JsonResponse
    {
        return $this->userClassapi->authJsonResponse($check);
    }
    ...
    public function postReportIssueFaqsList(Request $request)
    {
        try {
            ...
            $user_check = $this->userClassapi->checkUserAllow($request->get('provider_id'), $request->get('access_token'));
            if ($failed = $this->failIfAuthError($user_check)) {
                return $failed;
            }

            $provider_check = $this->resolveProviderCheck($request);
            if ($failed = $this->failIfAuthError($provider_check)) {
                return $failed;
            }
            $provider_data = $provider_check;

            // Determine language-specific column names
            $lang = $provider_data->language ?? 'en';
            ...
            return response()->json([
                'status' => 0,
                'message' => __('user_messages.351'), // Oops! Something went wrong. Please try again soon
                'message_code' => 351,
            ]);
        }
    }
    ...
    public function postUpdateReportIssue(Request $request)
    {
        try {
            ...
            if (($image_count < $min_images) || ($image_count > $max_images)) {
                ...
                return response()->json([
                    "status" => 0,
                    "message" => __('user_messages.352', ['min' => $min_images,'max' => $max_images]),
                    "message_code" => 352
                ]);
            }
            ...
            $report_issue = ReportIssues::query()
                ->select('report_issues.*','user_ride_booking.ride_no as order_no')
                ->leftJoin('user_ride_booking', 'report_issues.ride_id', '=', 'user_ride_booking.id') // Join with user_service_package_booking table details
            ...
            if ($report_issue != null) {
                ...
                if ($general_settings !=  Null){
                    ...
                    if ($general_settings->send_mail == 1) {
                        ...
                        // Get issue category
                        $issue_category = $report_issue->order_no != 0 ? $report_issue->order_no : "General Issue";
                        try {
                ...
                return response()->json([
                    "status" => 0,
                    "message" => __('user_messages.353'), // Sorry, we couldn’t find the details for the issue you reported
                    "message_code" => 353
                ]);
            }
        ...
        try{
            ...
            if ($report_id != null) {
                ...
                try {
                    ...
                    $file->move(public_path('/assets/images/report-issue/'), $file_new);
    ...
    public function postReportIssueDetails(Request $request)
    {
        try {
            ...
            $report_issue = ReportIssues::query()
                ->select('report_issues.*', 'user_ride_booking.ride_no')
                ->leftJoin('user_ride_booking', 'report_issues.ride_id', '=', 'user_ride_booking.id') // Join for get user_service_package_booking table details
    ...
    /**
     * Function for show report issue history of laundry service orders
     */
    public function postReportIssueHistory(Request $request)
    {
        try {
            ...
            $validator = Validator::make($request->all(), [
                ...
                "provider_type" => "required|numeric|in:1,2",// 0 = user, 3 = provider
            ...
            $query = ReportIssues::select(
                ...
                'user_ride_booking.ride_no','vehicle_services.name as service_name',
                DB::raw("(CASE WHEN vehicle_services.icon_name != '' THEN (concat('$service_icon_url','/',vehicle_services.icon_name)) ELSE '' END) as service_icon"),
                DB::raw("DATE_FORMAT(report_issues.created_at, '%Y-%m-%d %H:%i:%s') as report_issue_date_time")
            )
                ->leftJoin('user_ride_booking', 'report_issues.ride_id', '=', 'user_ride_booking.id')
                ->leftJoin('transport_vehicle_type', 'transport_vehicle_type.id', '=', 'user_ride_booking.vehicle_type_id')
                ->leftJoin('vehicle_services', 'vehicle_services.id', '=', 'transport_vehicle_type.service_id')
...
//            if ($request->provider_type != 2) {
                if ($request->general_issue_filter == 0) {
//                    $query->where('report_issues.ride_id','!=', 0);
...
//            }
            ...
            if ($request->has('status') && $request->status != "") {
                ...
                $query->whereIn('report_issues.status', collect([1, 2])->intersect($statuses)->values()->all());
            }
            ...
            $general_settings = request()->get('general_settings');
            // Get general report issue icon & name for driver app only
            $category_icon = url('/assets/images/report-issue/logo/' . $general_settings->general_report_issue_icon);
            $category_name = __('user_messages.373');

            $data = $report_issues->map(function ($issue) use ($category_icon, $category_name) {
                return [
                    "report_id" => $issue->id,
                    "reference_no" => $issue->reference_no,
                    "ride_no" => $issue->ride_no,
                    "category_name" => $issue->service_icon != Null ? ucwords(strtolower($issue->service_name)) : $category_name,
                    "category_icon" => $issue->service_icon != Null ? $issue->service_icon : $category_icon,
                    "status" => $issue->status,
    ...
    public function postGeneralReportIssueHistory(Request $request)
    {
        try {
            ...
            $validator = Validator::make($request->all(), [
                ...
                "provider_type" => "required|numeric|in:1",// 0 = user
                "filter_type" => "nullable|in:0,1,2,3,4,5",// 0 = all, 1 = today, 2 = last 7 days, 3 = last 30 days, 4 = this year, 5 = upcoming order, 6 = last 365 days
            ...
            $report_issue_history = $reportIssues->map(function ($issue) use ($general_settings) {
                return [
                    ...
                    "category_icon"          => url('/assets/images/report-issue/logo/' . $general_settings->general_report_issue_icon),
                    "category_name"          => __('user_messages.345'),
                    "status"                 => $issue->status,
                    "report_issue_date_time" => $issue->report_issue_date_time,
                ];
            });
    ...
    public function uploadChatPhoto(Request $request)
    {
        try {
            ...
            return response()->json([
                "status" => 0,
                "message" => __('user_messages.351'), // Fallback message
                "message_code" => 351,
            ]);
        } catch (\Exception $e) {
            // Handle unexpected errors
    ...
    public function deleteChatPhoto(Request $request)
    {
        try {
            ...
            try {
                ...
                return response()->json([
                    "status" => 0,
                    "message" => __('user_messages.351'), // General failure message
                    "message_code" => 351,
                ]);
            }
            ...
            return response()->json([
                "status" => 0,
                "message" => __('user_messages.351'), // General fallback error
                "message_code" => 351,
            ]);
        }
    }
