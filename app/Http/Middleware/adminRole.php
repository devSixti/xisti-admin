    ...
    public function handle($request, Closure $next)
    {
        ...
        if($this->admin_role == 1 || $this->admin_role == 2 || $this->admin_role == 3 ) {
            $this->admin_menu_list =AdminModule::query()->select('admin_module.id','admin_module.parent_id','admin_module.name','admin_module.module_name','admin_module.route_path','admin_module.image')
                ...
                ->get();
            if ($this->admin_menu_list->isNotEmpty()) {
                foreach ($this->admin_menu_list as $single_parent) {
                    $child_list = AdminModule::query()->select('admin_module.id','admin_module.parent_id','admin_module.name','admin_module.module_name','admin_module.route_path','admin_module.route_path_arr','admin_module.image')
                        ->where('admin_module.status','=',1)
        ...
        $request->attributes->add([
            ...
            'admin_main_menu_list' => $this->admin_main_menu_list,

            'admin_city_id' => $this->admin_city_id,
            'is_all_service' => $this->is_all_service,
            'admin_category_id_list' => $this->admin_category_id_list,
        ]);
        view()->composer('*',function($view) {
            $view->with('is_restrict_admin', $this->is_restrict_admin);
            ...
            $view->with('admin_city_id', $this->admin_city_id);
